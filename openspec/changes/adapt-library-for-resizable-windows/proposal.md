...
- Introduce one on-demand system inspector for entry detail and let SwiftUI adapt it between a trailing column and a sheet for the current presentation environment.
...
- Preserve focused entry, scroll position, selection, detail session, and active workflows across live window resizing while the system inspector adapts in place.
...
- `adaptive-library-experience`: Content-fit-driven Gallery layout, system-adaptive on-demand detail inspection, state preservation during resizing, and compatibility with the current iPhone experience.
...
- Requires one canonical detail presentation plus enum-driven workflow sheets instead of separate sheet and inspector hosts.
