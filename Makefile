...
test-sim:
	...
	@echo "Using simulator $(BOOTED_SIMULATOR_ID)"
	@echo "Running MyAnimeList tests..."
	@xcodebuild -quiet -project $(PROJECT) -scheme $(SCHEME) -configuration $(CONFIGURATION) -destination "id=$(BOOTED_SIMULATOR_ID)" test -only-testing:MyAnimeListTests -collect-test-diagnostics never && echo "** TEST SUCCEEDED **"
	@echo "MyAnimeList tests completed."
	@$(MAKE) --no-print-directory test-dataprovider
	@echo "All tests completed."
